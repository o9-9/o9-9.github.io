...
const normalizeApps = async (app) => {
	...
	const { [NON_APP_STORE_VERSION]: nonAppStoreLink, ...regularLinks } =
		Object.fromEntries(headerLinks);

	const links = {
		...regularLinks,
		...data.links,
		...(data.showSupportLink &&
			!data.isArchived && {
				Support: `/feedback?product=${encodeURIComponent(data.title)}`,
			}),
	};
	...
	let videos = await import.meta.glob("~/../public/apps/*/video*.mp4", {
		eager: false,
	});
	...
	let screenshots = await import.meta.glob(
		"~/../public/apps/*/screenshot*.{png,jpg}",
		{ eager: false },
	);
	...
	return {
		...
		hasIOSAppIcon:
			...
			((data.platforms.includes("iOS") || data.platforms.includes("watchOS")) &&
				!data.platforms.includes("macOS")),
		...
		screenshots,
		Content,
		olderVersionsUrl: data.repoUrl
			? `${data.repoUrl}#download`
			: `/${slug}#older-versions`,
		...(data.appStoreId && {
			appStoreUrl: `https://apps.apple.com/app/id${data.appStoreId}`,
		}),
		...(data.setappId && {
			setappUrl: `https://go.setapp.com/stp181?refAppID=${data.setappId}&utm_medium=vendor_program&utm_content=button`,
		}),
		feedbackNote,
	};
};
